...
-- 2. 教練`Q太郎` 的經驗年數為5年
UPDATE "COACH"
SET "experience_years" = "target"."years"
FROM "USER"
  JOIN (
    VALUES ('muscle@hexschooltest.io', 3),
      ('starplatinum@hexschooltest.io', 5)
) AS "target"("email", "years") ON "USER"."email" = "target"."email"
...
SELECT total_credits.user_id,
  (
    total_credits.total_purchased - COALESCE(used_credits.total_used, 0)
  ) AS remaining_credit
FROM total_credits
...
SELECT DISTINCT "COACH".experience_years AS "經驗年數",
...
SELECT "skill_info".name AS "專長名稱",
...
SELECT id,
  ...
  FROM "CREDIT_PACKAGE"
WHERE id IN (
    SELECT credit_package_id
    FROM "CREDIT_PURCHASE"
  )
)
SELECT "credit_package_info".name AS "組合包方案名稱",
...
WHERE EXTRACT(
    MONTH
    FROM "CREDIT_PURCHASE".created_at
  ) = 11
  AND EXTRACT(
    YEAR
    FROM "CREDIT_PURCHASE".created_at
